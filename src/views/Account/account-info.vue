<template>
  <DashboardHeader title="Your Profile" description="Home/routers" :icon="icon.accountIcon">
      ...
      <div
        ...
        <div
            ...
            <div class="relative inline-block">
              <div class="text-md text-slate-500">
                <button @click="handleSetActiveTab('profile')" class="text-[#464E5F] ml-3 font-medium" role="tab"
                  type="button">Your Profile</button>
              </div>
            </div>
          </div>
          <div class="relative px-3 py-3 listhover  rounded-md"
            ...
            <div class="relative inline-block">
              <div class="text-md text-slate-500">
                <button @click="handleSetActiveTab('changepassword')" class="text-[#464E5F] ml-3 font-medium">Change
                  Password</button>
              </div>
            </div>
          </div>
          <div class="relative px-3 py-3 listhover  rounded-md mb-3">
            ...
            <div class="relative inline-block">
              <div @click="handleLogout" class="text-md text-blue-600"><button class="text-[#464E5F] ml-3 font-medium">Logout</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="lg:col-span-3 asis-1/2 full-width" :class="[activeTab == 'profile' ? 'block ' : 'hidden']">
        <form @submit.prevent="submitForm">
          <div class="profile-sec full-width">
            <h2 class="font-medium md:text-[1.8rem] text-[1.4rem] md:mt-0 mt-5">Profile</h2>
            <div
        ...
        <div class="email-sec mt-8  full-width" :class="[activeTab == 'changepassword' ? 'd-block ' : 'd-none']">
          <h2 class="font-medium mt-6 inline-block md:text-[1.8rem] text-[1.4rem]">Change Email Address</h2>
          <div
      ...
      <div class="lg:col-span-3 asis-1/2  " :class="[activeTab == 'changepassword' ? 'block ' : 'hidden']">
        <form @submit.prevent="changePasswordForm">
          <div class="profile-sec">
            <h2 class="font-medium md:text-[1.8rem] text-[1.4rem] md:mt-0 mt-5">Change Password</h2>
            <div
...
export default defineComponent({
  ...
  data() {
    return {
      firstName: '',
      lastName: '',
      phone: '+77074739714', // this is static value for phone number because we don't have phone value in design
      email: '',
      new_password: '',
      old_password: '',
      confirm_password: '',
  ...
  methods: {
    ...
    async changePassword() {
      ...
      try {
        const response = await instance.post('auth/users/set_password/', formData);
        console.log(response.data);
      } catch (error) {
        console.error(error);
      }
